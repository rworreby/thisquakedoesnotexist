...
config_d = {
    ...
    'gan_file': 'gan_fno1d_3C.py',
    # ------------------------- output configuration -------------------
    'out_dir': '/scratch/mflorez/fourier/ffn1D/exp_3C/lr5',
    'print_every': 400,
    'save_every': 1,
}
...
# in case batch size is smaller than number of plots
if Nb<NPLT_MAX:
...
stl = "Real Accelerograms"
plot_waves_3C(x_r, DT, t_max=50.0, show_fig=True, fig_file=fig_file,stitle=stl, color='C0')
...
grf = GaussianRF_3C(1, Lt, alpha=ALPHA, tau=TAU, device=device)
z = grf.sample(NPLT_MAX)
print("z shape: ", z.shape)
z = z.permute(0, 2, 1)
z = z.detach().cpu()
# Make plot
fig_file = os.path.join(FIG_DIR, 'grf_3C.png')
stl = "GRF 1D 3-Components"
plot_waves_3C(z, DT, t_max=50.0, show_fig=True, fig_file=fig_file,stitle=stl, color='C4')
...
import gan_fno1d_3C
reload(gan_fno1d_3C)
from gan_fno1d_3C import Generator, Discriminator
# ----- test Discriminator-----
x_r, y_r = next(iter(train_loader))
# get discriminator
Nb = x_r.size(0)
...
plot_waves_3C(x_g, DT, t_max=50.0, show_fig=True, fig_file=fig_file,stitle=stl, color='C1')
...
for epoch in range(NUM_EPOCHS):
    ...
    stl = f"Epoch={epoch}"
    plot_waves_3C(x_g, DT, t_max=50.0, show_fig=False, fig_file=fig_file, stitle=stl, color='C1')
    # back to train mode
    G.train()
