...
config_d = {
    ...
    'gan_file': 'gan_fno1d_up.py',
    # ------------------------- output configuration -------------------
    #'out_dir': '/scratch/mflorez/fourier/ffn1D/exp/m64_lr5_2',
    'out_dir': '/scratch/mflorez/fourier/ffn1D/exp_up_1C/md128_lr5',
    'print_every': 400,
    'save_every': 1,
}
...
# Parameter for gaussian random field
GRF_LT = 200
grf = GaussianRF_1C(1, GRF_LT, alpha=ALPHA, tau=TAU, device=device)
...
plot_waves_1C(z, DT*5, t_max=50.0, show_fig=True, fig_file=fig_file,stitle=stl, color='C4')

#%%
# ----- Testing models ------
import gan_fno1d_up
reload(gan_fno1d_up)
from gan_fno1d_up import Generator, Discriminator
# ----- test Discriminator-----
x_r, y_r = next(iter(train_loader))
# get discriminator
Nb = x_r.size(0)
...
x_g = x_g.squeeze()
x_g = x_g.detach().cpu()
plot_waves_1C(x_g, DT, t_max=50.0, ylim=(-0.1,0.1), show_fig=True, fig_file=fig_file,stitle=stl, color='C1')
