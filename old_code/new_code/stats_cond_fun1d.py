...
BASE_DIR = '/scratch/mflorez/fourier/ffn1D/exp_4x/cond_kernel_c5'
...
# %%
# ----- Testing models ------
import gan_cond_fun1d
from gan_cond_fun1d import Generator, Discriminator
...
fig_file = os.path.join(OUT_FIG_DIR, f"gano_time_stats_{EPi}.pdf")
fig.tight_layout()
...
fig_file = os.path.join(OUT_FIG_DIR, f"gano_gan_tt_main.pdf")
#fig_file = os.path.join(TT_STATS_DIR, f"gano_gan_tt_main.pdf")
fig.savefig(fig_file,format='pdf')
plt.show()
...
fig_file = os.path.join(OUT_FIG_DIR, f"gano_freq_stats_all.pdf")
fig.tight_layout()
fig.savefig(fig_file,format='pdf')
#plt.show()
...
for idx, dist in DIST_DICT.items():
    hist_l2_plot(axs[idx], G, grf, dataset, dist, color_g='C1',device=device)
stitle = "GANO"
fig.suptitle(stitle, fontsize=10)
fig.supxlabel('l2 norm', fontsize=10)
fig.supylabel('Normalized Counts', fontsize=10)
fig_file = os.path.join(TT_STATS_DIR, f"gano_gan_l2_stats.pdf")
fig.savefig(fig_file,format='pdf')
fig.show()
