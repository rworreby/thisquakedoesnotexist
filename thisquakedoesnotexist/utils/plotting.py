...
def plot_syn_data_single(samples, dist, dt, lt, fig_dir):
    ...
    for j in range(5):
        ...
        ax[1].plot(tt[half_wl:-half_wl+1], conv_data, label='Centered Rolling Mean')
        ...
        ax[2].loglog(freq, s_ps, lw=0.5, label='Fourier Transformed Signal')
        ax[2].plot(freq[:-half_wl], rolling_mean, label='Centered Rolling Mean')
