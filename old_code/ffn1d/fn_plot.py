...
def hist_l2_plot(ax, G, grf, s_dat, distb, nplot_samp=512, n_bins=14, device=None, fontsize=9, xlabel_show=False, ylabel_show=False, color_g='C5'):
    """
    ...
    if stitle is not None:
        ax.set_title(stitle)
    ax.legend(['Data', 'Synthetic'], fontsize=fontsize, loc='upper right', )
    if xlabel_show:
        ax.set_xlabel('l2 sum', fontsize=fontsize)
    if ylabel_show:
        ax.set_ylabel('Normalized Counts', fontsize=fontsize)


def time_stats_plot(ax, G, grf, s_dat, distb, nplot_samp=256, device=None, fontsize=9, xlabel_show=False, ylabel_show=False, color_g='C2'):
    """
    ...
    if stitle is not None:
        ax.set_title(stitle)
    ax.legend(['Data', 'Synthetic'], fontsize=fontsize, loc='lower right', )
    if xlabel_show:
...
def fig_make_waves_1C(sfig, G, grf, s_dat, distb, nplot_samp = 20, t_max=50.0,  ylim=None, color='C1', left_frac=0.3, right_frac=0.7, show_labels=True, fontsize=9, device=None):
    """
        Make plot of waves
...
def make_val_figs_all(G, grf, s_dat, dist_dict, fig_file, figsize=(9, 36), Ni=2, Nj=3, iD1=0, iD2=1, fontsize=9, device=None):
    ...
    for idx, dist in dist_dict.items():
        time_stats_plot(axs0[idx], G, grf, s_dat, dist, fontsize=fontsize, device=device)
    ...
    # --- get data ---------
    fig_make_waves_1C(subfigs[1], G, grf, s_dat, dist1, nplot_samp=8, t_max=50.0, color='C1', fontsize=fontsize, device=device)
    fig_make_waves_1C(subfigs[2], G, grf, s_dat, dist2, nplot_samp=8, t_max=50.0, color='C1', fontsize=fontsize, device=device)

    # ----- Plot histograms ----
    axs1 = subfigs[3].subplots(nrows=Ni, ncols=Nj, sharey='row')
    # make plots
    for idx, dist in dist_dict.items():
        hist_l2_plot(axs1[idx], G, grf, s_dat, dist, device=device)
