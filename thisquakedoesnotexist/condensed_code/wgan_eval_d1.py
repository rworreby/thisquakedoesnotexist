...
# costum utilities
from thisquakedoesnotexist.models.gan_d1 import Discriminator, Generator

# make sure class ais reloaded everytime the cell is executed
from thisquakedoesnotexist.condensed_code.dataUtils_v1 import SeisData, plot_waves_3C
...
config_d = {
    ...
    'out_dir': 'thisquakedoesnotexist/data/output',
    'print_every': 1,
    'save_every': 1,
}
...
def init_gan_conf(conf_d):
    ...
    # gan_file = os.path.join('./', 'gan_d1.py')
...
for i_epoch in range(num_epochs):
    ...
    for i_batch in range(N_train_btot):
        ...
        ### ------------- end batch --------------
    # ----- End training epoch ------
    print(f"Training loop {i_epoch} done.")
    ...
    for i_batch in range(N_val_btot):
        ...
        g_val_loss += g_loss.item()
            ### --------------  END GENERATOR STEP ------------------------
    print(f"Validation Generator epoch {i_epoch} done.")
