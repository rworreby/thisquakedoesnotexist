...
def kernel(n_chan, up_dim=16):
    """
        two layers functional kernel
    """
    layers = nn.Sequential(
            nn.Linear(n_chan, up_dim), torch.nn.GELU(),
    #        nn.Linear(up_dim, up_dim), torch.nn.GELU(),
            nn.Linear(up_dim, n_chan),
        )
    return layers
...
class Discriminator(nn.Module):
    def __init__(self, modes, width, lt=1000, kernel_dim=16, padding=9):
    ...
    def forward(self, x):
        ...
        # function convolved wih Kernel
        # appy kernel to grid
        kx = self.knet(grid)
        # print("self.knet(grid)", kx.shape)
        # convolver function with kernel
        x = torch.einsum('bik,bik->bk', kx, x)/self.lt
        return x
